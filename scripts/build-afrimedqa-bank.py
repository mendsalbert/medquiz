...
IMAGE_JSON = DATA_DIR / "clinical-image-quizzes.json"
IMAGE_BANK_JSON = DATA_DIR / "clinical-image-quizzes.bank.json"
...
def image_bank_from_extract() -> list[dict]:
    if not IMAGE_JSON.exists():
        return []
    raw = json.loads(IMAGE_JSON.read_text())
    bank = []
    for quiz in raw:
        item = dict(quiz)
        quiz_id = str(item["id"])
        if not quiz_id.endswith("-images"):
            item["id"] = f"{quiz_id}-images"
        if "(clinical images)" not in item["title"]:
            item["title"] = f"{item['title']} (clinical images)"
        desc = str(item.get("description", "")).rstrip(".")
        item["description"] = (
            f"{desc}. Image-based AfriMedEval items kept alongside AfriMed-QA v2."
        )
        bank.append(item)
    return bank
...
def write_outputs(afrimed_quizzes: list[dict]) -> None:
    ...
    image_bank = image_bank_from_extract()
    IMAGE_BANK_JSON.write_text(json.dumps(image_bank, ensure_ascii=False, indent=2) + "\n")
...
"""
    ...
    total_q = sum(q["questionCount"] for q in afrimed_quizzes) + sum(
        q["questionCount"] for q in image_bank
    )
    print(f"Wrote {OUT_JSON} ({len(afrimed_quizzes)} AfriMed-QA quizzes)")
    print(f"Wrote {IMAGE_BANK_JSON} ({len(image_bank)} clinical-image quizzes)")
    print(f"Wrote {OUT_TS}")
    print(
        f"Total quizzes: {len(afrimed_quizzes) + len(image_bank)}, questions: {total_q}"
    )
